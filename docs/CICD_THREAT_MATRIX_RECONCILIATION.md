...
| 06 | Execution; Persistence | Inject code to IaC configuration | `existing-partial` | `PSB-IAC-001` provides approved modules, resolved-plan Policy as Code, fail-closed decisions, provider-side enforcement requirements, and drift handling; `PSB-BUILD-001` limits build impact. Alternate deployment paths and malicious provider execution remain residual risks. | `PSB-DETECT-001` supplies the planned integrity-verified scanner interface. |
...
| 08 | Execution; Persistence | Inject bad dependency | `existing-partial` | `PSB-DEPS-001..003` enforce cooldown, install-script denial, frozen resolution, and artifact integrity. They do not establish maintainer intent or vulnerability-free content. | `PSB-DEPS-004` adds change review and `PSB-DETECT-001` adds verified scanning. |
...
| 11 | Execution (Production) | Deploy modified applications or server images to production environment | `existing-partial` | `PSB-BUILD-003` creates platform provenance, `PSB-REL-001` verifies artifact identity and provenance, and `PSB-REL-002` publishes provenance. Deployment-time admission is not yet demonstrated. | The unreserved artifact-signing-generation gap and planned `PSB-CONTAINER-001` remain relevant. |
