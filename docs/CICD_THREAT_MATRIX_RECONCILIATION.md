...
| Audit logging and control-plane monitoring | `PSB-CICD-008` requires an independent reviewer to compare each in-scope privileged change with the live provider event and current setting. | Complete GitHub event coverage, independent retention, organization-wide drift, alert delivery, and provider compromise remain external evidence; repository templates do not verify them. |
