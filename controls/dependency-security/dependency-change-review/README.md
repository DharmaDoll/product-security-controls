...
dependencyだけを見てtransitive変更を見落とすreview、`warn-only`やoptional checkの設定ミス、dependency graph／
...
Default branchへmergeするpull request、GitHub dependency graphが対応するmanifest／lockfile、baseとheadの
dependency snapshot、追加・更新されたdirect／transitive dependency、version、scope、manifest／lockfile、known
...
GitHub Dependency Review Actionがdependency graphのbase／head snapshotを比較し、changed dependencyとknown
vulnerabilityをPR上へ表示する。Referenceはruntime、development、unknown scopeの`high`以上をjob failureにする。
...
| Graph／Action failure | Unsupported manifest、partial graph、API／runner failureをempty safe diffと解釈する | 実際には未評価のdependency変更がreview済みとしてmergeされる | `ERROR`／`NOT_CHECKED`としてrequired checkを満たさない |
...
- `NOT_CHECKED`: Ecosystem、manifest、transitive graph、provider resultを確認できない。低riskという意味ではない。
...
- Workflow fileのlocal lint: Live dependency graphやactive rulesetを証明しない。
...
Supported graphを取得できない場合、Reference workflowをcopyしてempty diffを得てもcontrol導入にはならない。その
...
- Provider outageやpartial graphを組織がどう扱うかは決まらない。
...
- Repository administrator: Dependency graph、workflow、default／release branch ruleset、required check、
...
- Diffが表示されない場合はmanifest support、dependency graph、base／head、lockfile commit、transitive coverageを確認する。
...
外す。Dependency graph、branch protection、他security workflowを一括無効化しない。代替がなければ状態を
...
- Static repository testはlive graph、provider availability、active rulesetを証明しない。
